...
export function AirportDashboardMobileScreen() {
  const [rows, setRows] = useState<any[]>([]);
  useEffect(() => {
    fetch('/api/v1/airport-dashboard').then((r) => r.json()).then((j) => setRows(j.data?.items || []));
  }, []);
  return (
    <View style={styles.container}>
      <Text style={styles.title}>AirportDashboard</Text>
      <FlatList
        data={rows}
