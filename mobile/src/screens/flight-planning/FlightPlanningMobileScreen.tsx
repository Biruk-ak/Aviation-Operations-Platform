...
export function FlightPlanningMobileScreen() {
  const [rows, setRows] = useState<any[]>([]);
  useEffect(() => {
    fetch('/api/v1/flight-planning').then((r) => r.json()).then((j) => setRows(j.data?.items || []));
  }, []);
  return (
    <View style={styles.container}>
      <Text style={styles.title}>FlightPlanning</Text>
      <FlatList
        data={rows}
