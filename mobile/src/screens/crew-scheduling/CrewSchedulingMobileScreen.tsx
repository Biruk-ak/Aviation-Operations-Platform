...
export function CrewSchedulingMobileScreen() {
  const [rows, setRows] = useState<any[]>([]);
  useEffect(() => {
    fetch('/api/v1/crew-scheduling').then((r) => r.json()).then((j) => setRows(j.data?.items || []));
  }, []);
  return (
    <View style={styles.container}>
      <Text style={styles.title}>CrewScheduling</Text>
      <FlatList
        data={rows}
