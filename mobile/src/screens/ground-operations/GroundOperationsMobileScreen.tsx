...
export function GroundOperationsMobileScreen() {
  const [rows, setRows] = useState<any[]>([]);
  useEffect(() => {
    fetch('/api/v1/ground-operations').then((r) => r.json()).then((j) => setRows(j.data?.items || []));
  }, []);
  return (
    <View style={styles.container}>
      <Text style={styles.title}>GroundOperations</Text>
      <FlatList
        data={rows}
