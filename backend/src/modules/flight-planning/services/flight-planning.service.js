...
const { FlightPlanningModel } = require('../models/flight-planning.model');
const { validateFlightPlanning } = require('../validators/flight-planning.validator');
const { FlightPlanningEvents } = require('../events/flight-planning.events');
const { FlightPlanningRepository } = require('../repositories/flight-planning.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class FlightPlanningService {
  constructor(options = {}) {
    this.model = options.model || new FlightPlanningModel();
    this.repo = options.repo || new FlightPlanningRepository();
    this.events = options.events || new FlightPlanningEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateFlightPlanning(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateFlightPlanning(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateFlightPlanning(input, 'create').length === 0, errors: validateFlightPlanning(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'flight-planning', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
...
module.exports = { FlightPlanningService };
