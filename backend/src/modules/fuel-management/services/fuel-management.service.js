...
const { FuelManagementModel } = require('../models/fuel-management.model');
const { validateFuelManagement } = require('../validators/fuel-management.validator');
const { FuelManagementEvents } = require('../events/fuel-management.events');
const { FuelManagementRepository } = require('../repositories/fuel-management.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class FuelManagementService {
  constructor(options = {}) {
    this.model = options.model || new FuelManagementModel();
    this.repo = options.repo || new FuelManagementRepository();
    this.events = options.events || new FuelManagementEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateFuelManagement(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateFuelManagement(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateFuelManagement(input, 'create').length === 0, errors: validateFuelManagement(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'fuel-management', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
...
module.exports = { FuelManagementService };
