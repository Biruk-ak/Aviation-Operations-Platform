...
const { AirportDashboardModel } = require('../models/airport-dashboard.model');
const { validateAirportDashboard } = require('../validators/airport-dashboard.validator');
const { AirportDashboardEvents } = require('../events/airport-dashboard.events');
const { AirportDashboardRepository } = require('../repositories/airport-dashboard.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class AirportDashboardService {
  constructor(options = {}) {
    this.model = options.model || new AirportDashboardModel();
    this.repo = options.repo || new AirportDashboardRepository();
    this.events = options.events || new AirportDashboardEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateAirportDashboard(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateAirportDashboard(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateAirportDashboard(input, 'create').length === 0, errors: validateAirportDashboard(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'airport-dashboard', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
...
module.exports = { AirportDashboardService };
