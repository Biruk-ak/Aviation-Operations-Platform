...
const { MobileAppsModel } = require('../models/mobile-apps.model');
const { validateMobileApps } = require('../validators/mobile-apps.validator');
const { MobileAppsEvents } = require('../events/mobile-apps.events');
const { MobileAppsRepository } = require('../repositories/mobile-apps.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class MobileAppsService {
  constructor(options = {}) {
    this.model = options.model || new MobileAppsModel();
    this.repo = options.repo || new MobileAppsRepository();
    this.events = options.events || new MobileAppsEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateMobileApps(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateMobileApps(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateMobileApps(input, 'create').length === 0, errors: validateMobileApps(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'mobile-apps', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
...
module.exports = { MobileAppsService };
