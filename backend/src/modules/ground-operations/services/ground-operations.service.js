...
const { GroundOperationsModel } = require('../models/ground-operations.model');
const { validateGroundOperations } = require('../validators/ground-operations.validator');
const { GroundOperationsEvents } = require('../events/ground-operations.events');
const { GroundOperationsRepository } = require('../repositories/ground-operations.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class GroundOperationsService {
  constructor(options = {}) {
    this.model = options.model || new GroundOperationsModel();
    this.repo = options.repo || new GroundOperationsRepository();
    this.events = options.events || new GroundOperationsEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateGroundOperations(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateGroundOperations(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateGroundOperations(input, 'create').length === 0, errors: validateGroundOperations(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'ground-operations', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
...
module.exports = { GroundOperationsService };
