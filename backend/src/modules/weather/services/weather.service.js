...
const { validateWeather } = require('../validators/weather.validator');
const { WeatherEvents } = require('../events/weather.events');
...
class WeatherService {
  constructor(options = {}) {
    this.model = options.model || new WeatherModel();
    this.repo = options.repo || new WeatherRepository();
    this.events = options.events || new WeatherEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateWeather(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateWeather(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateWeather(input, 'create').length === 0, errors: validateWeather(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'weather', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
