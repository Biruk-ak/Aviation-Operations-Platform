...
const { validateNavigation } = require('../validators/navigation.validator');
const { NavigationEvents } = require('../events/navigation.events');
...
class NavigationService {
  constructor(options = {}) {
    this.model = options.model || new NavigationModel();
    this.repo = options.repo || new NavigationRepository();
    this.events = options.events || new NavigationEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateNavigation(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateNavigation(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateNavigation(input, 'create').length === 0, errors: validateNavigation(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'navigation', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
