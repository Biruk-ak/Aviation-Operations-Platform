...
const { validateMaintenance } = require('../validators/maintenance.validator');
const { MaintenanceEvents } = require('../events/maintenance.events');
...
class MaintenanceService {
  constructor(options = {}) {
    this.model = options.model || new MaintenanceModel();
    this.repo = options.repo || new MaintenanceRepository();
    this.events = options.events || new MaintenanceEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateMaintenance(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateMaintenance(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateMaintenance(input, 'create').length === 0, errors: validateMaintenance(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'maintenance', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
