...
const { validateTicketing } = require('../validators/ticketing.validator');
const { TicketingEvents } = require('../events/ticketing.events');
const { TicketingRepository } = require('../repositories/ticketing.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class TicketingService {
  constructor(options = {}) {
    this.model = options.model || new TicketingModel();
    this.repo = options.repo || new TicketingRepository();
    this.events = options.events || new TicketingEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateTicketing(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateTicketing(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateTicketing(input, 'create').length === 0, errors: validateTicketing(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'ticketing', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
