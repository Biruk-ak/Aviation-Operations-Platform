...
const { validateAircraft } = require('../validators/aircraft.validator');
const { AircraftEvents } = require('../events/aircraft.events');
...
class AircraftService {
  constructor(options = {}) {
    this.model = options.model || new AircraftModel();
    this.repo = options.repo || new AircraftRepository();
    this.events = options.events || new AircraftEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateAircraft(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateAircraft(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateAircraft(input, 'create').length === 0, errors: validateAircraft(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'aircraft', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
