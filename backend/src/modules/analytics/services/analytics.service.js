...
const { AnalyticsEvents } = require('../events/analytics.events');
...
class AnalyticsService {
  constructor(options = {}) {
    this.model = options.model || new AnalyticsModel();
    this.repo = options.repo || new AnalyticsRepository();
    this.events = options.events || new AnalyticsEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateAnalytics(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateAnalytics(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateAnalytics(input, 'create').length === 0, errors: validateAnalytics(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'analytics', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
