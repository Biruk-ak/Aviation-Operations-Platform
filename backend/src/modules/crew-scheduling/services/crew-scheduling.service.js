...
const { CrewSchedulingModel } = require('../models/crew-scheduling.model');
const { validateCrewScheduling } = require('../validators/crew-scheduling.validator');
const { CrewSchedulingEvents } = require('../events/crew-scheduling.events');
const { CrewSchedulingRepository } = require('../repositories/crew-scheduling.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class CrewSchedulingService {
  constructor(options = {}) {
    this.model = options.model || new CrewSchedulingModel();
    this.repo = options.repo || new CrewSchedulingRepository();
    this.events = options.events || new CrewSchedulingEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateCrewScheduling(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validateCrewScheduling(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validateCrewScheduling(input, 'create').length === 0, errors: validateCrewScheduling(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'crew-scheduling', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
...
module.exports = { CrewSchedulingService };
