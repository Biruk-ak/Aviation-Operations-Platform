...
const { PilotsModel } = require('../models/pilots.model');
const { validatePilots } = require('../validators/pilots.validator');
const { PilotsEvents } = require('../events/pilots.events');
const { PilotsRepository } = require('../repositories/pilots.repository');
const { audit } = require('../../shared/audit');
const { AppError } = require('../../shared/errors');

class PilotsService {
  constructor(options = {}) {
    this.model = options.model || new PilotsModel();
    this.repo = options.repo || new PilotsRepository();
    this.events = options.events || new PilotsEvents();
  }
  ...
  async create(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validatePilots(input, 'create');
  ...
  async update(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      const errors = validatePilots(input, 'update');
      if (errors.length) throw new AppError('VALIDATION', 'Invalid payload', errors);
  ...
  async validate(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { valid: validatePilots(input, 'create').length === 0, errors: validatePilots(input, 'create') };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
  ...
  async getDependencies(input = {}, ctx = {}) {
    const started = Date.now();
    try {
      return { module: 'pilots', dependsOn: ['organizations', 'users', 'airports'], dependents: [] };
    } catch (err) {
      err.durationMs = Date.now() - started;
      throw err;
    }
