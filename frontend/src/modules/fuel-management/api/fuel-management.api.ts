...
import type { FuelManagement, FuelManagementCreateInput, FuelManagementFilter, FuelManagementListResponse, FuelManagementUpdateInput } from '../types/fuel-management.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/fuel-management';

export const fuelManagementApi = {
  async list(filter: FuelManagementFilter = {}): Promise<FuelManagementListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<FuelManagement> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: FuelManagementCreateInput): Promise<FuelManagement> {
    return http.post(BASE, input);
  },
  async update(input: FuelManagementUpdateInput): Promise<FuelManagement> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<FuelManagementListResponse> {
