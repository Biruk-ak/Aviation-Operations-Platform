...
import type { MobileApps, MobileAppsCreateInput, MobileAppsFilter, MobileAppsListResponse, MobileAppsUpdateInput } from '../types/mobile-apps.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/mobile-apps';

export const mobileAppsApi = {
  async list(filter: MobileAppsFilter = {}): Promise<MobileAppsListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<MobileApps> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: MobileAppsCreateInput): Promise<MobileApps> {
    return http.post(BASE, input);
  },
  async update(input: MobileAppsUpdateInput): Promise<MobileApps> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<MobileAppsListResponse> {
