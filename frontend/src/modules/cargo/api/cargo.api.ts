...
import type { Cargo, CargoCreateInput, CargoFilter, CargoListResponse, CargoUpdateInput } from '../types/cargo.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/cargo';

export const cargoApi = {
  async list(filter: CargoFilter = {}): Promise<CargoListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Cargo> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: CargoCreateInput): Promise<Cargo> {
    return http.post(BASE, input);
  },
  async update(input: CargoUpdateInput): Promise<Cargo> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<CargoListResponse> {
