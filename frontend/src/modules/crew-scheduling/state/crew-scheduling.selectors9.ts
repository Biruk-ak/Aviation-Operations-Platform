...
import type { CrewScheduling } from '../types/crew-scheduling.types';

export function selectCrewSchedulingView9_1(items: CrewScheduling[] = [], query = '') {
...
export function selectCrewSchedulingView9_2(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_3(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_4(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_5(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_6(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_7(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_8(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_9(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_10(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_11(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_12(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_13(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_14(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_15(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_16(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_17(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_18(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_19(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_20(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_21(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_22(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_23(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_24(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_25(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_26(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_27(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_28(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_29(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_30(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_31(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_32(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_33(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_34(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_35(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_36(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_37(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_38(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_39(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_40(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_41(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_42(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_43(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_44(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_45(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_46(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_47(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_48(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_49(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_50(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_51(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_52(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_53(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_54(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_55(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_56(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_57(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_58(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectCrewSchedulingView9_59(items: CrewScheduling[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
