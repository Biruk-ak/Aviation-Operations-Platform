...
import type { FlightPlanning, FlightPlanningCreateInput, FlightPlanningFilter, FlightPlanningListResponse, FlightPlanningUpdateInput } from '../types/flight-planning.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/flight-planning';

export const flightPlanningApi = {
  async list(filter: FlightPlanningFilter = {}): Promise<FlightPlanningListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<FlightPlanning> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: FlightPlanningCreateInput): Promise<FlightPlanning> {
    return http.post(BASE, input);
  },
  async update(input: FlightPlanningUpdateInput): Promise<FlightPlanning> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<FlightPlanningListResponse> {
