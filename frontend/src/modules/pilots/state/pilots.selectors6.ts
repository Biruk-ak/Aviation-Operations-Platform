...
export function selectPilotsView6_1(items: Pilots[] = [], query = '') {
...
export function selectPilotsView6_2(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_3(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_4(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_5(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_6(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_7(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_8(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_9(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_10(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_11(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_12(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_13(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_14(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_15(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_16(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_17(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_18(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_19(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_20(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_21(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_22(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_23(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_24(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_25(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_26(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_27(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_28(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_29(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_30(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_31(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_32(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_33(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_34(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_35(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_36(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_37(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_38(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_39(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_40(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_41(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_42(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_43(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_44(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_45(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_46(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_47(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_48(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_49(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_50(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_51(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_52(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_53(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_54(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_55(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_56(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_57(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_58(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
...
export function selectPilotsView6_59(items: Pilots[] = [], query = '') {
  const q = query.toLowerCase();
  return items
    .filter((x) => !q || `${x.code} ${x.name} ${x.status}`.toLowerCase().includes(q))
    .map((x, idx) => ({
