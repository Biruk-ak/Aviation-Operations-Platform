...
import type { Pilots, PilotsCreateInput, PilotsFilter, PilotsListResponse, PilotsUpdateInput } from '../types/pilots.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/pilots';

export const pilotsApi = {
  async list(filter: PilotsFilter = {}): Promise<PilotsListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Pilots> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: PilotsCreateInput): Promise<Pilots> {
    return http.post(BASE, input);
  },
  async update(input: PilotsUpdateInput): Promise<Pilots> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<PilotsListResponse> {
