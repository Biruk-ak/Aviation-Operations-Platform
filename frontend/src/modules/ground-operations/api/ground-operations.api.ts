...
import type { GroundOperations, GroundOperationsCreateInput, GroundOperationsFilter, GroundOperationsListResponse, GroundOperationsUpdateInput } from '../types/ground-operations.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/ground-operations';

export const groundOperationsApi = {
  async list(filter: GroundOperationsFilter = {}): Promise<GroundOperationsListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<GroundOperations> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: GroundOperationsCreateInput): Promise<GroundOperations> {
    return http.post(BASE, input);
  },
  async update(input: GroundOperationsUpdateInput): Promise<GroundOperations> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<GroundOperationsListResponse> {
