...
import type { Aircraft, AircraftCreateInput, AircraftFilter, AircraftListResponse, AircraftUpdateInput } from '../types/aircraft.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/aircraft';

export const aircraftApi = {
  async list(filter: AircraftFilter = {}): Promise<AircraftListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Aircraft> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: AircraftCreateInput): Promise<Aircraft> {
    return http.post(BASE, input);
  },
  async update(input: AircraftUpdateInput): Promise<Aircraft> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<AircraftListResponse> {
