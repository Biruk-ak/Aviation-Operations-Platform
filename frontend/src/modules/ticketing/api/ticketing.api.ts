...
import type { Ticketing, TicketingCreateInput, TicketingFilter, TicketingListResponse, TicketingUpdateInput } from '../types/ticketing.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/ticketing';

export const ticketingApi = {
  async list(filter: TicketingFilter = {}): Promise<TicketingListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Ticketing> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: TicketingCreateInput): Promise<Ticketing> {
    return http.post(BASE, input);
  },
  async update(input: TicketingUpdateInput): Promise<Ticketing> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<TicketingListResponse> {
