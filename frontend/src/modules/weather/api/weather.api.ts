...
import type { Weather, WeatherCreateInput, WeatherFilter, WeatherListResponse, WeatherUpdateInput } from '../types/weather.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/weather';

export const weatherApi = {
  async list(filter: WeatherFilter = {}): Promise<WeatherListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Weather> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: WeatherCreateInput): Promise<Weather> {
    return http.post(BASE, input);
  },
  async update(input: WeatherUpdateInput): Promise<Weather> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<WeatherListResponse> {
