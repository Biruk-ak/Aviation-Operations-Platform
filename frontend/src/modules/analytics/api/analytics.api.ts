...
import type { Analytics, AnalyticsCreateInput, AnalyticsFilter, AnalyticsListResponse, AnalyticsUpdateInput } from '../types/analytics.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/analytics';

export const analyticsApi = {
  async list(filter: AnalyticsFilter = {}): Promise<AnalyticsListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Analytics> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: AnalyticsCreateInput): Promise<Analytics> {
    return http.post(BASE, input);
  },
  async update(input: AnalyticsUpdateInput): Promise<Analytics> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<AnalyticsListResponse> {
