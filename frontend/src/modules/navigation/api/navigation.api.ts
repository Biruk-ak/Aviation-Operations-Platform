...
import type { Navigation, NavigationCreateInput, NavigationFilter, NavigationListResponse, NavigationUpdateInput } from '../types/navigation.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/navigation';

export const navigationApi = {
  async list(filter: NavigationFilter = {}): Promise<NavigationListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Navigation> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: NavigationCreateInput): Promise<Navigation> {
    return http.post(BASE, input);
  },
  async update(input: NavigationUpdateInput): Promise<Navigation> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<NavigationListResponse> {
