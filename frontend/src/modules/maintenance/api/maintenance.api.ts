...
import type { Maintenance, MaintenanceCreateInput, MaintenanceFilter, MaintenanceListResponse, MaintenanceUpdateInput } from '../types/maintenance.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/maintenance';

export const maintenanceApi = {
  async list(filter: MaintenanceFilter = {}): Promise<MaintenanceListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<Maintenance> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: MaintenanceCreateInput): Promise<Maintenance> {
    return http.post(BASE, input);
  },
  async update(input: MaintenanceUpdateInput): Promise<Maintenance> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<MaintenanceListResponse> {
