...
import type { AirportDashboard, AirportDashboardCreateInput, AirportDashboardFilter, AirportDashboardListResponse, AirportDashboardUpdateInput } from '../types/airport-dashboard.types';
import { http } from '../../lib/http';

const BASE = '/api/v1/airport-dashboard';

export const airportDashboardApi = {
  async list(filter: AirportDashboardFilter = {}): Promise<AirportDashboardListResponse> {
    return http.get(BASE, { params: filter });
  },
  async getById(id: string): Promise<AirportDashboard> {
    return http.get(`${BASE}/${id}`);
  },
  async create(input: AirportDashboardCreateInput): Promise<AirportDashboard> {
    return http.post(BASE, input);
  },
  async update(input: AirportDashboardUpdateInput): Promise<AirportDashboard> {
    return http.put(`${BASE}/${input.id}`, input);
  },
  async remove(id: string): Promise<{ success: boolean }> {
    return http.delete(`${BASE}/${id}`);
  },
  async search(query: string): Promise<AirportDashboardListResponse> {
